...
Superficies (recorrelas todas): Inicio (partidos del día de todas las competiciones; flechas ← → para navegar día a día, incluido cambio de mes y de año; días sin partidos; horarios y resultados; links a competición y a equipo; **dos vistas conmutables de la misma lista —por torneo (default) y por horario— con un control que alterna entre ellas y recuerda la elección entre sesiones**); Competición (todos los partidos de una competición: Primera A, Libertadores, Sudamericana; lista agrupada por fecha con subgrupos por horario; volver atrás); Equipo (fixture propio de un equipo, incluido el favorito; lista sin agrupar, cada partido con su fecha, hora y competición; volver atrás). El equipo favorito viene predefinido fuera de la UI: no hay selector ni forma de cambiarlo desde la app, así que no busques uno ni lo reportes como faltante.
...
Flujos que rompen: doble/rápido submit (¿duplicados?), spam de clicks en acciones, guardar/navegar durante carga, borrar una entidad EN USO (borrado a cargo del usuario: ¿lo impide con mensaje?, ¿histórico consistente?), editar+cancelar (¿descarta y reabre con valores originales?). Propios de Fulbo: spam de las flechas ← → en Inicio saltando muchos días seguidos (¿se pisan las respuestas?, ¿queda el día que corresponde?), navegar a un día muy lejano en el futuro o en el pasado (¿empty prolijo o error?), drill-down encadenado partido → competición → equipo → volver atrás varias veces (¿el atrás vuelve donde corresponde?), entrar a una competición o a un equipo sin partidos (¿empty prolijo?), agotar la cuota de 100 requests/día de API-Football (la pantalla tiene que quedar **idéntica** a una con cuota disponible: mismos datos de caché, sin aviso, mensaje, badge ni ninguna bifurcación visual — si aparece algún tratamiento propio de cuota agotada, ESO es el bug), backend caído o respuesta lenta (¿mensaje con reintento, no pantalla blanca?), partido en juego o con resultado parcial (¿se muestra sin NaN ni campos vacíos?), spam del conmutador de vista de Inicio alternando muchas veces seguidas (¿la lista queda coherente con la vista que muestra el control?, ¿queda algún grupo duplicado o a medio reagrupar?), cambiar de vista y después navegar de día con las flechas (¿la vista elegida se mantiene o se reinicia al default?, ¿y al ir a Competición o Equipo y volver, y al recargar?), `localStorage` no disponible o bloqueado —modo privado, storage deshabilitado, cuota llena— (¿la app sigue funcionando con la vista default, sin error visible ni pantalla rota, y el conmutador sigue alternando dentro de la sesión?).
